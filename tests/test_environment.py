...
class TestDetectEnvironment:
    ...
    def test_returns_venv_when_pyvenv_cfg_missing(
...
class TestDetectWindowsPythonSource:
    ...
    def test_store_python(self, monkeypatch):
        result = self._detect_with_exe(
            monkeypatch, r"C:\Users\user\AppData\Local\Microsoft\WindowsApps\python.exe"
        )
        assert result == "store"
    ...
    def test_conda_python(self, monkeypatch):
        result = self._detect_with_exe(
            monkeypatch, r"C:\Users\user\miniconda3\python.exe"
        )
        assert result == "conda"

    def test_pyenv_python(self, monkeypatch):
        result = self._detect_with_exe(
            monkeypatch, r"C:\Users\user\.pyenv\pyenv-win\versions\3.12.0\python.exe"
        )
        assert result == "pyenv"

    def test_scoop_python(self, monkeypatch):
        result = self._detect_with_exe(
            monkeypatch, r"C:\Users\user\scoop\apps\python\current\python.exe"
        )
        assert result == "scoop"
    ...
    def test_unknown_python(self, monkeypatch):
        result = self._detect_with_exe(monkeypatch, r"C:\CustomPython\python.exe")
