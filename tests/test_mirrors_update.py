...
from cnpip.mirrors import (
    DEFAULT_MIRRORS,
    update_mirrors_from_remote,
    REMOTE_MIRRORS_URLS,
)
...
def fake_user_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        mirrors_module, "USER_MIRRORS_FILE", config_dir / "mirrors.json"
    )
...
def test_url_list_prefers_china_accessible_cdn():
    ...
    raw_index = next(
        i for i, u in enumerate(REMOTE_MIRRORS_URLS) if "raw.githubusercontent.com" in u
    )
    jsdelivr_index = next(
        i for i, u in enumerate(REMOTE_MIRRORS_URLS) if "jsdelivr.net" in u
    )
...
def test_first_url_success(fake_user_config, monkeypatch):
    monkeypatch.setattr(
        mirrors_module, "_fetch_mirrors_json", lambda url, timeout=5: FAKE_MIRRORS
    )
