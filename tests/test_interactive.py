...
class TestRunInteractiveSet:
    ...
    def test_enter_uses_default(self, monkeypatch, capsys):
        monkeypatch.setattr(
            module, "scan_available_tools", lambda: [("pip", ""), ("uv", "")]
        )
        monkeypatch.setattr(module, "detect_environment", lambda: "system")
    ...
    def test_failed_tool_gives_nonzero_exit(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            module, "apply_mirror_to_tool", lambda tool, name, url, args: False
        )
    ...
    def test_failed_batch_rolls_back_completed_tools(self, monkeypatch, capsys):
        monkeypatch.setattr(
            module, "scan_available_tools", lambda: [("uv", ""), ("pdm", "")]
        )
        monkeypatch.setattr(module, "detect_environment", lambda: "system")
    ...
    def test_failed_batch_rolls_back_pip_without_unpack_error(
    ...
    def test_adapter_exception_also_rolls_back_completed_tools(
        self, monkeypatch, capsys
    ):
        monkeypatch.setattr(
            module, "scan_available_tools", lambda: [("uv", ""), ("pdm", "")]
...
class TestCliTrigger:
    ...
    def test_explicit_tool_flag_skips_interactive(
